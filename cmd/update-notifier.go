...
func prepareUpdateMessage(downloadURL string, older time.Duration) string {
	...
	if globalCLIContext.JSON {
		return fmt.Sprintf("You are running an older version of AnnaStore released %s, update: %s", newerThan, downloadURL)
	}
...
func colorizeUpdateMessage(updateString string, newerThan string) string {
	msgLine1Fmt := " You are running an older version of AnnaStore released %s "

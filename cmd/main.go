...
func newApp(name string) *cli.App {
	...
	app.HideHelpCommand = true // Hide `help, h` command, we already have `annastore --help`.
	app.Commands = commands
	app.CustomAppHelpTemplate = minioHelpTemplate
	app.CommandNotFound = func(ctx *cli.Context, command string) {
		console.Printf("‘%s’ is not a annastore sub-command. See ‘annastore --help’.\n", command)
...
func startupBanner(banner io.Writer) {
	fmt.Fprintln(banner, color.Blue("Copyright:")+color.Bold(" 2021-%s GuinsooLab Annastore", CopyrightYear))
